...
title: Agents
icon: i-lucide-bot
description: The same thirty-five tools over MCP, the Pi extension and the OMP extension, what they hold back, and how tokens and hosts stay out of a model's hands.
---

## Thirty-five tools, three surfaces

The MCP server, the Pi extension and the OMP extension all call the same executors in `src/tool-operations.ts`, so they answer identically. Fix one, fixed everywhere. The tools mirror the ten resources:
...
Every tool takes a `platform` of `github`, `gitlab` or `gitea` plus the owner, repo, number, tag or username the call needs.
