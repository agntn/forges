...
So `@agntn/forges` puts one abstract `Provider` in front of all of it. Ten resources, same method shapes, one `Repository`, one `Issue`, one `PullRequest`, one `Thread`. And it finds the token for you.
...
`createProvider` is async because it imports the provider module on demand: once the token is resolved, only the code for that one platform loads.

## Same call, any platform

```ts
const gitlab = await createProvider("gitlab"); // GITLAB_TOKEN, then glab
const gitea = await createProvider("gitea", { baseURL: "https://codeberg.org" }); // GITEA_TOKEN
...
forge.ciRuns; //                 list(owner, repo, { branch? }), listJobs(owner, repo, runId), readJobLog(owner, repo, jobId)
forge.commits; //                list(owner, repo, { ref?, path?, since?, until? }), get(owner, repo, sha)
forge.releases; //               list(owner, repo), get(owner, repo, tag), create, update(owner, repo, tag, input)
forge.issues; //                 list, search, get, create, listComments, getComment
forge.pullRequests; //           list, listFiles, listChecks, listReviews, getReview, search, get, create, listComments, getComment
...
A 404 from GitHub and a 404 from GitLab are both `NotFoundError`. 401 is `AuthenticationError`, a 403 that is not a rate limit is `PermissionError`, 429 is `RateLimitError`. Everything else is a plain `ForgesError` with the status. The message keeps the reason the platform put in the error body, like Gitea's `user does not exist` for an owner it doesn't know, cut to 200 characters. An HTML error page adds nothing to it. Something a provider simply does not have, like code search on Gitea, is a `ForgesError` with status 501 and a sentence saying so. No pretending.

## Next

- [Authentication](/guide/auth): the token chain and self hosted endpoints.
- [Repositories](/guide/repositories), [Issues](/guide/issues), [Pull requests](/guide/pull-requests), [Review threads](/guide/threads), [Commits, CI and releases](/guide/commits).
- [Agents](/guide/agents): 48 tools over MCP, Pi and OMP.
