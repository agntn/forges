...
- Writes need one on every platform: `issues.create`, `issues.createComment`, `pullRequests.create`, `pullRequests.update`, `pullRequests.createComment`, `threads.reply`, `threads.resolve`, `threads.unresolve`.
