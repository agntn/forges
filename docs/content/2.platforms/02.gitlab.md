...
- The merge request pipeline rows have no `name`, only `GET /projects/:id/pipelines/:pipeline_id` has it, so `listChecks` costs one more request per pipeline on the page, sent to the project the pipeline ran in. A fork merge request runs in the fork, and the target project answers 404 for that pipeline.
