...
- The merge request pipeline rows have no `name`, only `GET /projects/:id/pipelines/:pipeline_id` has it, so `listChecks` reads each pipeline on the page once more, from the project it ran in, and keeps that read in the cache because a name never changes. A fork merge request runs in the fork, and the target project answers 404 for that pipeline. A name lookup that fails keeps its row, named `pipeline`, with a warning on stderr.
