...
const github = await createProvider("github"); // GH_TOKEN, GITHUB_TOKEN, then gh auth token
const enterprise = await createProvider("github", { baseURL: "https://github.example.com/api/v3" });
```
...
| Resource                     | Endpoint                                                                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| repos                        | `GET /repos/:owner/:repo`, `GET /orgs/:owner/repos`, `GET /users/:owner/repos`, `GET /user/repos` for the viewer                                 |
| issues                       | `GET /repos/:owner/:repo/issues`, `GET /search/issues`, `POST /repos/:owner/:repo/issues`, `PATCH …/issues/:number`, `…/issues/:number/comments` |
| pull requests                | `GET /repos/:owner/:repo/pulls`, `…/pulls/:number/files`, `…/commits/:sha/check-runs`, `…/commits/:sha/status`                                   |
| issue and pull request edits | `PATCH …/pulls/:number`, `POST`/`DELETE …/issues/:number/assignees`, `POST …/issues/:number/labels`, `DELETE …/labels/:name`                     |
| commits                      | `GET /repos/:owner/:repo/commits`, `…/commits/:sha`, `…/compare/:base...:head`                                                                   |
| CI runs                      | `GET /repos/:owner/:repo/actions/runs`, `…/actions/runs/:id/jobs`, `…/actions/jobs/:id`, `…/actions/jobs/:id/logs`                               |
| releases                     | `GET /repos/:owner/:repo/releases`, `…/releases/tags/:tag`, `POST …/releases`, `PATCH …/releases/:id`                                            |
...
- `releases/tags/:tag` serves published releases only. A draft is found by walking the newest releases, five pages of a hundred, and a draft older than that is a `NotFoundError` by tag.
