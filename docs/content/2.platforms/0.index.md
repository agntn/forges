...
Every platform answers the same ten resources with the same shapes. What changes underneath is the API, the header the token travels in, how a page says there is a next one, and which operations exist at all. The pages are short on purpose. The part worth reading is the gotcha list on each one.
